          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid w-full max-w-xl gap-3 rounded-3xl border border-border bg-background p-5 shadow-sm md:grid-cols-2">
              ...
              <div className="rounded-2xl border border-border bg-muted/20 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">Draft → review → send → CRM writeback</p>
              </div>
              <div className="rounded-2xl border border-border bg-muted/20 p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                  Focus
                ...
                <p className="mt-1 text-xs text-muted-foreground">Prefill → validate → preview → submit</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          {workflowCards.map((card) => (
            ...
            >
              ...
              <h2 className="mt-3 text-2xl font-bold tracking-tight text-foreground">{card.title}</h2>
              <p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">{card.description}</p>
              <div className="mt-6">
                <Link
                  href={card.href}
