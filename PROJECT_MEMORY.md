...
- **Suspense Boundaries & Query Params:** Extensive use of `React.Suspense` is required around components that consume `useSearchParams`. List pages (Leads, Contacts, Tasks) use URL parameters (`?q=`, `?page=`, `?status=`) for advanced search, filtering, and server-side `take`/`skip` pagination. We recently implemented a `MultiSelectFilter` component that pushes array-based search params (e.g., `?status=NEW&status=QUALIFIED`) to the URL, which Prisma then parses into `in` operator arrays to filter tables.
